...
namespace rs {
V8InspectorClient::V8InspectorClient(v8::Isolate* isolate, std::mutex& mutex,
                                     std::condition_variable& run_condition):
  m_isolate(isolate), m_mutex(mutex), m_runCondition(run_condition),
  m_ioc{1} {
}
...
void V8InspectorClient::connectSession() {
  ...
  tcp::acceptor acceptor{m_ioc, {address, static_cast<unsigned short>(9005)}};
...
void V8InspectorClient::createInspector() {
  ...
  m_session = m_inspector->connect(1, m_channel.get(),
...
bool V8InspectorClient::dispatchProtocolMessage() {
  ...
  if (!m_messageQueue.empty()) {
    v8_inspector::StringView protocolMessage;
    std::string message = m_messageQueue.front();
    m_messageQueue.pop();
    protocolMessage = convertToStringView(message);
...
void V8InspectorClient::runMessageLoopOnPause(int contextGroupId) {
  ...
  while (m_paused) {
    while (dispatchProtocolMessage() || v8::platform::PumpMessageLoop(platform, m_isolate)) {
    }
    {
      std::unique_lock lock(m_mutex);
      m_runCondition.wait_for(lock, std::chrono::milliseconds(10), [this]() {
        return !m_messageQueue.empty();
      });
    }
  }
