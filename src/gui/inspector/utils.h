...
static inline std::string convertToString(v8::Isolate* isolate,
    ...
    v8::Local<v8::String> message = (
        stringView.is8Bit()
            ? v8::String::NewFromOneByte(isolate,
                                         reinterpret_cast<const uint8_t*>(stringView.
                                             characters8()), v8::NewStringType::kNormal,
                                         length)
            : v8::String::NewFromTwoByte(isolate,
                                         reinterpret_cast<const uint16_t*>(stringView.
                                             characters16()),
...
static inline v8_inspector::StringView convertToStringView(
...
static inline v8::Local<v8::Object> parseJson(
    const v8::Local<v8::Context>& context, const std::string& json)
{
    v8::MaybeLocal<v8::Value> value_ = v8::JSON::Parse(
        context,
        v8::String::NewFromUtf8(context->GetIsolate(), &json[0],
...
static inline std::string getPropertyFromJson(v8::Isolate* isolate, v8::Local<v8::Context> context,
                                              const v8::Local<v8::Object>&
                                              jsonObject,
                                              const std::string& propertyName)
{
    v8::Local<v8::Value> property = jsonObject->Get(
                                                    context,
...
static std::string getExceptionMessage(v8::Isolate* isolate,
...
static bool compileScript(const v8::Local<v8::Context>& context,
                          ...
                          const v8::TryCatch& tryCatch)
{
    ...
    v8::MaybeLocal<v8::Script> script_ = v8::Script::Compile(
        context, source, &scriptOrigin);
    if (!script_.IsEmpty())
    {
...
static bool executeScript(const v8::Local<v8::Context>& context,
...
static bool runScript(const v8::Local<v8::Context>& context,
                      const std::string& filePath)
{
    ...
    if (!compileScript(context, source, filePath, script, tryCatch))
    {
        ...
        std::cerr << "Exception: " << getExceptionMessage(
            isolate_, tryCatch.Exception()) << std::endl;
        return false;
    }
